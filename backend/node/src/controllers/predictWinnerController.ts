...
import { predictMensWinner, predictWomensWinner, validatePredictionInput } from '../models/predictWinnerModel.js';
...
export const predictWomensWinnerController = async (req: Request, res: Response): Promise<void> => {
    try {
        const requestData: PlayerInput = req.body;
        ...
        console.log(`Processing womens prediction request for ${player1} vs ${player2}`);
        console.log(`Match details: ${surface} surface, ${round} round, best of ${bestOf}`);
        console.log(`Odds: ${player1} (${player1Odds}), ${player2} (${player2Odds})`);
        console.log('Player 1 stats:', player1Stats);
        console.log('Player 2 stats:', player2Stats);

        const prediction = await predictWomensWinner(
            player1, 
            player2, 
            surface, 
            round, 
        ...
        console.error("Error in predictwomenswinner route:", error);
