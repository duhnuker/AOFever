...
interface AOMensDoublesFinalsData {
  id: number;
  year: number;
...
const AOMensDoubles = () => {
  const [finalsData, setFinalsData] = useState<AOMensDoublesFinalsData[]>([]);
  ...
  useEffect(() => {
    const fetchFinalsData = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/ausopenfinals/aomensdoublesfinals');
    ...
    <Box className='bg-gradient-to-t from-zinc-900 to-zinc-700 min-h-screen'>
      ...
      <Container size="4" className='px-2 sm:px-4 md:px-6'>
        <Flex justify="center" className='my-8 sm:my-14'>
          ...
          >
            Australian Open Men's Doubles Finals
          </Heading>
        </Flex>
